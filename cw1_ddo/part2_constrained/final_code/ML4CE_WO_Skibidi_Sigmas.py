def ML4CE_WO_Skibidi_Sigmas_algorithm(
        problem,
        bounds,  
        budget, 
        rep
    ...
    def spline_optimiser(
        ...
        # Initialize points and use the observations to generate a sample of function and constraint values
        ...
        for iteration in range(budget-4):
            ...
            con1_new = con1(x_new) # Evaluate true constraint 1
            con2_new = con2(x_new) # Evaluate true constraint 2

            # Update samples and refit splines
            X_init = np.vstack((X_init, x_new))
            Y_init = np.append(Y_init, f_new)
            con_surr1 = np.append(con_surr1, con1_new)
            con_surr2 = np.append(con_surr2, con2_new)
            x, f_best = x_new, f_new
            # Break if budget is exhausted
            if iteration + 1 >= budget-4:
                break
    ...
    cids = ['02093393', '02101922', '02028380', '02022119']
