...
def controller(x: np.array, f: callable, sp: np.array, env: callable, u_prev: np.array, evaluation_interval=0.5, total_duration=1.0) -> np.array:
    ...
    def adjust_weights(error, error_rate, Q_base, R_base):
        """
        Adjust Q and R dynamically with faster response to error and error rate changes.
        Parameters:
        ...
        Returns:
        tuple: Updated Q and R.
        """
        # Exponential scaling for faster responsiveness
        alpha = 2.0  # Scaling factor for Q
        beta = 1.0   # Scaling factor for R

        # Direct scaling by error and rate of error change
    ...
    for step in range(num_steps):
        ...
        # Dynamically adjust Q and R
        Q, R = adjust_weights(error, error_rate,Q_base,R_base)
